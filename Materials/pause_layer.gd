...
@onready var evil_voice: AudioStreamPlayer = game.get_node("IntroOverlay/EvilVoice")
...
func _setup_quit_confirm() -> void:
	# Полноэкранный блокировщик кликов — перекрывает слоты, тотемы и всё UI под диалогом
	input_blocker = ColorRect.new()
	input_blocker.anchor_left   = 0.0
	input_blocker.anchor_right  = 1.0
	input_blocker.anchor_top    = 0.0
	input_blocker.anchor_bottom = 1.0
	input_blocker.offset_left   = 0.0
	input_blocker.offset_right  = 0.0
	input_blocker.offset_top    = 0.0
	input_blocker.offset_bottom = 0.0
	input_blocker.color = Color(0.0, 0.0, 0.0, 0.0) # прозрачный, но ловит клики
