...
@export var dim_alpha: float = 0.14
...
func _show_combo_hit(token: int, hit: Dictionary) -> void:
	...
	var combo_mult: int = int(hit.get("combo_multiplier", 0))

	if combo_mult > 0:
		_combo_name_label.text = "%s  x%d" % [combo_name, combo_mult]
	else:
		_combo_name_label.text = combo_name
	_combo_value_label.text = "+%d" % win_amount

	_combo_popup.visible = true
	_combo_popup.modulate.a = 0.0
	_combo_popup.scale = Vector2(0.84, 0.84)
	_combo_value_label.modulate = Color(1.0, 1.0, 1.0, 0.0)
	_combo_name_label.modulate = Color(1.0, 1.0, 1.0, 0.0)
	_neon_t = 0.0

	var intro: Tween = create_tween()
	intro.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	intro.parallel().tween_property(_combo_popup, "scale", Vector2.ONE, 0.20)
	intro.parallel().tween_property(_combo_popup, "modulate:a", 1.0, 0.12)
	intro.parallel().tween_property(_combo_value_label, "modulate:a", 1.0, 0.10)
	intro.parallel().tween_property(_combo_name_label, "modulate:a", 1.0, 0.10)
	...
	pulse.tween_property(_combo_popup, "scale", Vector2(1.03, 1.03), 0.08)
	pulse.tween_property(_combo_popup, "scale", Vector2.ONE, 0.08)
	...
	outro.parallel().tween_property(_combo_popup, "scale", Vector2(1.07, 1.07), 0.12)
	outro.parallel().tween_property(_combo_popup, "modulate:a", 0.0, 0.12)
...
func _show_total(token: int, total_win: int) -> void:
	...
	_total_title_label.text = "ВЫИГРЫШ"
	_total_value_label.text = "+%d" % total_win

	_total_popup.visible = true
	_total_popup.modulate.a = 0.0
	_total_popup.scale = Vector2(0.76, 0.76)
	_total_title_label.modulate = Color(1.0, 1.0, 1.0, 0.0)
	_total_value_label.modulate = Color(1.0, 1.0, 1.0, 0.0)
	_neon_t = 0.0

	var intro: Tween = create_tween()
	intro.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	intro.parallel().tween_property(_total_popup, "scale", Vector2.ONE, 0.24)
	intro.parallel().tween_property(_total_popup, "modulate:a", 1.0, 0.16)
	intro.parallel().tween_property(_total_value_label, "modulate:a", 1.0, 0.10)
	intro.parallel().tween_property(_total_title_label, "modulate:a", 1.0, 0.10)
	await intro.finished
	if token != _sequence_token:
		return
	...
	var pulse: Tween = create_tween()
	pulse.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	pulse.tween_property(_total_popup, "scale", Vector2(1.04, 1.04), 0.10)
	pulse.tween_property(_total_popup, "scale", Vector2.ONE, 0.10)
	await pulse.finished
	if token != _sequence_token:
		return
	...
	var outro: Tween = create_tween()
	outro.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	outro.parallel().tween_property(_total_popup, "scale", Vector2(1.10, 1.10), 0.14)
	outro.parallel().tween_property(_total_popup, "modulate:a", 0.0, 0.14)
	await outro.finished
	if token != _sequence_token:
		return
...
func _apply_visuals() -> void:
	...
	_apply_panel_style(_combo_popup, Vector2(560.0, 220.0), Vector2(0.0, -6.0), Color(0.0, 0.0, 0.0, 0.0), Color(0.0, 0.0, 0.0, 0.0), 0)
	_apply_panel_style(_total_popup, Vector2(640.0, 260.0), Vector2(0.0, -6.0), Color(0.0, 0.0, 0.0, 0.0), Color(0.0, 0.0, 0.0, 0.0), 0)

	_apply_label_style(_combo_name_label, 28, Color(1.0, 0.96, 0.20, 1.0), Color(0.28, 0.0, 0.26, 1.0), 6)
	_apply_label_style(_combo_value_label, 90, Color(1.0, 0.92, 0.22, 1.0), Color(1.0, 0.08, 0.92, 1.0), 12)
	_apply_label_style(_total_title_label, 34, Color(1.0, 0.98, 0.34, 1.0), Color(0.35, 0.02, 0.32, 1.0), 8)
	_apply_label_style(_total_value_label, 102, Color(1.0, 0.96, 0.24, 1.0), Color(1.0, 0.08, 0.92, 1.0), 13)

	if _combo_name_label != null:
		_combo_name_label.set_anchors_preset(Control.PRESET_TOP_WIDE)
		_combo_name_label.offset_left = 0.0
		_combo_name_label.offset_top = 12.0
		_combo_name_label.offset_right = 0.0
		_combo_name_label.offset_bottom = 62.0

	if _combo_value_label != null:
		_combo_value_label.set_anchors_preset(Control.PRESET_FULL_RECT)
		_combo_value_label.offset_left = 0.0
		_combo_value_label.offset_top = 40.0
		_combo_value_label.offset_right = 0.0
		_combo_value_label.offset_bottom = 0.0

	if _total_title_label != null:
		_total_title_label.set_anchors_preset(Control.PRESET_TOP_WIDE)
		_total_title_label.offset_left = 0.0
		_total_title_label.offset_top = 8.0
		_total_title_label.offset_right = 0.0
		_total_title_label.offset_bottom = 64.0

	if _total_value_label != null:
		_total_value_label.set_anchors_preset(Control.PRESET_FULL_RECT)
		_total_value_label.offset_left = 0.0
		_total_value_label.offset_top = 56.0
		_total_value_label.offset_right = 0.0
		_total_value_label.offset_bottom = 0.0
...
func _update_neon_anim() -> void:
	var phase: float = 0.5 + 0.5 * sin(_neon_t * 5.2)
	var edge_phase: float = 0.5 + 0.5 * sin(_neon_t * 4.1 + 1.2)
	var glow_phase: float = 0.5 + 0.5 * sin(_neon_t * 6.0 + 0.6)

	if _combo_value_label != null and _combo_popup != null and _combo_popup.visible:
		var combo_main := Color(1.0, 0.84 + 0.12 * phase, 0.20 + 0.10 * edge_phase, _combo_value_label.modulate.a)
		var combo_outline := Color(1.0, 0.06 + 0.20 * glow_phase, 0.92, 1.0)
		_combo_value_label.add_theme_color_override("font_color", combo_main)
		_combo_value_label.add_theme_color_override("font_outline_color", combo_outline)

	if _total_value_label != null and _total_popup != null and _total_popup.visible:
		var total_main := Color(1.0, 0.88 + 0.10 * phase, 0.22 + 0.10 * edge_phase, _total_value_label.modulate.a)
		var total_outline := Color(1.0, 0.05 + 0.20 * glow_phase, 0.92, 1.0)
		_total_value_label.add_theme_color_override("font_color", total_main)
		_total_value_label.add_theme_color_override("font_outline_color", total_outline)
