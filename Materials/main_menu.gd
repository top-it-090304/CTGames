...
func _on_play_pressed():
	...
	if SaveSystem.has_save():
		# В режиме "Продолжить" — просто переходим на сцену игры
		get_tree().change_scene_to_file("res://Materials/game.tscn")
	else:
		# В режиме "Играть" — удаляем старое сохранение и заходим
...
func _on_quit_pressed():
	...
	if SaveSystem.has_save():
		# В режиме "Начать заново" — стираем сохранение и запускаем игру
		SaveSystem.delete_save()
		get_tree().change_scene_to_file("res://Materials/game.tscn")
	else:
		# В режиме "Выйти" — запускается твоя старая пасхалка с текстом
		play_locked = true
...
# ─────────────────────────────────────────────
#  Конфигурация (Оставляем твой код без изменений)
# ─────────────────────────────────────────────
