...
func configure_visuals(new_icon_size: Vector2, new_separation: int) -> void:
	icon_size = new_icon_size
	icon_separation = new_separation
	if box == null or margin == null:
		return
	_style_icons()
	_h = _detect_height()
	if _h <= 0.0:
...
func _ensure_icon_controls() -> void:
	if box == null:
		return
	if box.get_child_count() >= 6:
		return

	while box.get_child_count() < 6:
		var icon := TextureRect.new()
		icon.name = "Icon%d" % (box.get_child_count() + 1)
		icon.mouse_filter = Control.MOUSE_FILTER_IGNORE
		icon.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
		box.add_child(icon)

func _seed_icons_from_pool() -> void:
	_ensure_icon_controls()
	if _symbol_pool.is_empty():
		return
	for child: Node in box.get_children():
